...
# ToDo: Resize of Playing Field in FullScreen
class PlayingField(QWidget):
    def __init__(self):
        ...
        image_path = "res/controlsNoRGB.png"
    ...
    def update_preview_column(self, preview_column):
        print(f"update_preview_column with param {preview_column}")
        current_player = None
        try:
            current_player = self.find_player_by_id(self.controller.current_playerID)
        except:
            pass
    ...
    def find_player_by_id(self, playerID):
        print(f"Finding Player by ID: {playerID}")
        found_player = next((player for player in self.controller.players if player.playerID == playerID), None)
        return found_player
